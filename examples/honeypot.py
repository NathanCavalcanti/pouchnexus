...
PORTS = {2222: "SSH", 2223: "Telnet", 3306: "MySQL", 8080: "HTTP-Alt", 8443: "HTTPS-Alt"}
...
def send_to_soc(attacker_ip, port, flags_str):
    ...
    try:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ SOC: {attacker_ip} -> {port} [{flags_str}] ({attack_desc}) | Status: {r.status_code}")
