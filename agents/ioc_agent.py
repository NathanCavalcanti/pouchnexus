...
def enrich_with_virustotal(iocs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "hashes" in iocs:
        ...
        all_hashes = []
        max_len = (
            max(len(sha256_list), len(sha1_list), len(md5_list))
            if iocs.get("hashes")
            else 0
        )
        ...
        if all_hashes:
            print(
                f"[VirusTotal] Checking {min(len(all_hashes), max_hashes)} hash(es)..."
            )
            ...
            for h in all_hashes:
                ...
                if report.get("error"):
                    print(
                        f"[VirusTotal] Error for hash {h[:16]}...: {report.get('error')}"
                    )
                    continue
                ...
                if report:
                    ...
                    hashes_checked += 1
                    print(
                        f"[VirusTotal] ✓ Hash {h[:16]}... - Detection: {report.get('malicious_count', 0)}/{report.get('total_engines', 0)}"
                    )
    ...
    if "ips" in iocs and isinstance(iocs["ips"], list):
        ...
        for ip in iocs["ips"]:
            ...
            ips_checked += 1
            print(
                f"[VirusTotal] ✓ IP {ip} - Detection: {report.get('malicious_count', 0)}/{report.get('total_engines', 0)}, Reputation: {report.get('reputation', 0)}"
            )
            time.sleep(15)  # Rate limiting
    ...
    if "urls" in iocs and isinstance(iocs["urls"], list):
        ...
        for url in iocs["urls"]:
            ...
            urls_checked += 1
            print(
                f"[VirusTotal] ✓ URL - Detection: {report.get('malicious_count', 0)}/{report.get('total_engines', 0)}"
            )
            time.sleep(15)  # Rate limiting
    ...
    if "domains" in iocs and isinstance(iocs["domains"], list):
        ...
        for domain in iocs["domains"]:
            ...
            domains_checked += 1
            print(
                f"[VirusTotal] ✓ Domain {domain} - Detection: {report.get('malicious_count', 0)}/{report.get('total_engines', 0)}"
            )
            time.sleep(15)  # Rate limiting
